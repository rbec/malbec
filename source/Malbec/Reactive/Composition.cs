...
namespace Malbec.Reactive
{
  public static class Composition
  {
    ...
    #region Functions
    public static IExp<Δ0, T> Function<TΔX, TX, T>(IFunction<TΔX, Δ0, TX, T> f, IExp<TΔX, TX> x)
      => new FunctionExp<TΔX, Δ0, TX, T, T>(f, x);

    public static IExp<Δ0, T> Function<TΔX, TΔY, TX, TY, T>(IFunction<TΔX, TΔY, Δ0, TX, TY, T> f, IExp<TΔX, TX> x, IExp<TΔY, TY> y)
      => new FunctionExp<TΔX, TΔY, Δ0, TX, TY, T, T>(f, x, y);

    public static IExp<Δ1, IReadOnlyList<TItem>> Function<TΔX, TX, TItem, TInternal>(IFunction<TΔX, Δ1, TX, TInternal> f, IExp<TΔX, TX> x) where TInternal : IReadOnlyList<TItem>
      => new FunctionExp<TΔX, Δ1, TX, IReadOnlyList<TItem>, TInternal>(f, x);

    public static IExp<Δ1, IReadOnlyList<TItem>> Function<TΔX, TΔY, TX, TY, TItem, TInternal>(IFunction<TΔX, TΔY, Δ1, TX, TY, TInternal> f, IExp<TΔX, TX> x, IExp<TΔY, TY> y) where TInternal : IReadOnlyList<TItem>
      => new FunctionExp<TΔX, TΔY, Δ1, TX, TY, IReadOnlyList<TItem>, TInternal>(f, x, y);

    public static IExp<Δ1, IReadOnlyList<TItem>> Function<TΔX, TΔY, TΔZ, TX, TY, TZ, TItem, TInternal>(IFunction<TΔX, TΔY, TΔZ, Δ1, TX, TY, TZ, TInternal> f, IExp<TΔX, TX> x, IExp<TΔY, TY> y, IExp<TΔZ, TZ> z) where TInternal : IReadOnlyList<TItem>
      => new FunctionExp<TΔX, TΔY, TΔZ, Δ1, TX, TY, TZ, IReadOnlyList<TItem>, TInternal>(f, x, y, z);

    public static IExp<Δ0, T> Function<TX, T>(Func<TX, T> f, IExp<Δ0, TX> x)
      => new FunctionExp<Δ0, Δ0, TX, T, T>(new Function<TX, T>(f), x);

    public static IExp<Δ0, T> Function<TX, TY, T>(Func<TX, TY, T> f, IExp<Δ0, TX> x, IExp<Δ0, TY> y)
    ...
    public static IExp<Δ0, TItem> Fold<TItem>(Func<TItem, TItem, TItem> f, IExp<Δ1, IReadOnlyList<TItem>> x)
      => Function(new FoldFunction<TItem>(f), x);

    public static IExp<Δ1, IReadOnlyList<TItem>> Scan<TItem>(Func<TItem, TItem, TItem> f, IExp<Δ1, IReadOnlyList<TItem>> x)
      => Function<Δ1, IReadOnlyList<TItem>, TItem, ScanList<TItem>>(new ScanFunction<TItem>(f), x);
    ...
    public static IExp<Δ0, int> LowerBound<TItem, TOrder>(IExp<Δ1, IReadOnlyList<TItem>> x, IExp<Δ0, TItem> y) where TOrder : struct, IOrdering<TItem>
      => Function(new LowerBoundFunction<TItem, TOrder>(), x, y);
    ...
    public static IExp<Δ0, T?> Option<T>(IExp<Δ0, bool> x, IExp<Δ0, T> y) where T : struct
      => Invoke(Choose(new ConstantExp<Δ0, T?>(null), Function(AsNullable, y), x));

    public static IExp<Δ1, IReadOnlyList<T>> OptionToList<T>(IExp<Δ0, T?> x) where T : struct
      => Function<Δ0, T?, T, List<T>>(new OptionToListFunction<T>(), x);

    public static IExp<Δ0, T> Choose<T>(T x, T y, IExp<Δ0, bool> z)
      => Function(new ChooseFunction<T>(x, y), z);

    public static IExp<Δ0, int> Count<T>(IExp<Δ1, IReadOnlyList<T>> z)
      => Function(new CountFunction<T>(), z);
  }
}